...
# -------------------------------- 用于存储weights------------------------
    def save_weights(self, filepath):
        weights = {
            "conv1": self.conv1.get_weights(),
            "conv2": self.conv2.get_weights(),
            "fc1": self.fc1.get_weights(),
            "fc2": self.fc2.get_weights(),
            "fc3": self.fc3.get_weights()
        }
    ...
    def load_weights(self, filepath):
        ...
        self.conv2.set_weights(weights["conv2"])
        self.fc1.set_weights(weights["fc1"])
        self.fc2.set_weights(weights["fc2"])
        self.fc3.set_weights(weights["fc3"])
...
def preprocess_image(image_path):
    ...
    image = cp.array(image, dtype=cp.float16)
    return image

def predict(image_path, model, weights_path):
    model.load_weights(weights_path)
    image = preprocess_image(image_path)
    output = model.forward(image)
    prediction = cp.argmax(output, axis=1)
    return prediction.get().item()
#---------------------------------------------------------------------------------
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        with tqdm.tqdm(total=train_images.shape[0], desc=f"Epoch {epoch + 1}/{epochs}") as pbar:
            for i in range(0, train_images.shape[0], batch_size):
                ...
                total_correct += cp.sum(predictions == y_batch)
                total_samples += y_batch.shape[0]

                pbar.set_postfix({"Loss": f"{cp.mean(cp.array(losses)).item():.4f}", 
                                  "Acc": f"{(total_correct / total_samples).item():.4f}"})
                pbar.update(batch_size)
        avg_loss = cp.mean(cp.array(losses)).item()
        accuracy = (total_correct / total_samples).item()
        print(f"\nEpoch {epoch+1}/{epochs}, Loss: {avg_loss:.4f}, Accuracy: {accuracy:.4f}, Learning Rate: {lr}")
        sys.stdout.flush()
    
    # 保存模型权重
    model.save_weights('lenet5_weights.cp')
    # 测试
    correct = 0
    ...
    # predicted_digit = predict(image_path, model, 'lenet5_weights.cp')
